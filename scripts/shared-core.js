...
class SharedCore {
    ...
    getEventSanityFlags(event, context = {}) {
        ...
        // 9. junk-title — the title is navigation/CTA link text scraped as an
    ...
    // -------------------------------------------------------------------------
    // CALENDAR HYGIENE (STRICTLY REPORT-ONLY)
    //
    // During the buggy period the scraper wrote occurrence SINGLES for events
    // whose recurring series the owner has since saved via the ICS export —
    // leaving stale one-offs ("SUNDAY BEER BUST Aug 9" written before the
    // weekly series existed) sitting next to the series that now covers them.
    // Doctrine forbids the scraper from deleting anything (the phone owner is
    // the only writer; destructive changes need positive verification), so
    // this machinery only BUILDS A CHECKLIST: when a run's analysis has
    // positively matched saved series (_seriesMatch stamps), list the
    // wide-window calendar singles those series appear to supersede. Deletion
    // stays manual, in the Calendar app.
    //
    // Fail closed at every step: a false "delete me" suggestion is worse than
    // a missed one. Anything uncertain — unparseable/unsupported RRULE, an
    // anchor we cannot trust, a date before the observed series start — is
    // OMITTED, never guessed. Two finding kinds:
    //   'superseded'  the single's date IS generated by the confirmed series'
    //                 recurrence rule — the series already covers that night.
    //   'off-pattern' the single's identity matches a confirmed series but
    //                 its date is NOT generated (or is EXDATE-excluded) by
    //                 the rule — labeled separately because it might be a
    //                 genuinely special night (the wrong-Sunday beer bust).
    // Never listed at all: series instances/anchors themselves, and
    // single-occurrence OVERRIDES (notes overrideUid/overrideRecurrenceId —
    // the slot-host mechanism writes those deliberately).
    // -------------------------------------------------------------------------

    // Entry point (called by the orchestrator AFTER prepareEventsForCalendar,
    // so the write path is byte-identical with or without it). Reads the
    // confirmed-series stamps off the analyzed events, re-reads the adapter's
    // per-run wide-window cache, and returns findings. Read-only; any error
    // returns what was found so far and never throws into the run.
    async collectCalendarHygieneFindings(analyzedEvents, calendarAdapter) {
        const findings = [];
        try {
            if (!Array.isArray(analyzedEvents) || analyzedEvents.length === 0) return findings;
            if (!calendarAdapter || typeof calendarAdapter.getWideWindowCalendarEvents !== 'function') {
                return findings;
            }
            // Confirmed saved series this run matched, grouped by calendar.
            const byCalendar = new Map();
            for (const event of analyzedEvents) {
                const stamp = event && event._seriesMatch;
                if (!stamp || !stamp.identifier || !stamp.calendarName) continue;
                const calendarName = String(stamp.calendarName);
                if (!byCalendar.has(calendarName)) {
                    byCalendar.set(calendarName, { city: event.city || '', series: new Map() });
                }
                const bucket = byCalendar.get(calendarName);
                if (!bucket.city && event.city) bucket.city = event.city;
                const identifier = String(stamp.identifier).trim();
                if (identifier && !bucket.series.has(identifier)) {
                    bucket.series.set(identifier, { ...stamp, identifier, city: event.city || '' });
                }
            }
            for (const [calendarName, bucket] of byCalendar) {
                if (bucket.series.size === 0) continue;
                let lookup = null;
                try {
                    lookup = await calendarAdapter.getWideWindowCalendarEvents({ city: bucket.city || 'default' });
                } catch (error) {
                    lookup = null; // fail closed: no candidates, no claims
                }
                const candidates = lookup && Array.isArray(lookup.events) ? lookup.events : [];
                if (candidates.length === 0) continue;
                let publishedRecords = null;
                try {
                    if (typeof calendarAdapter.getPublishedCalendarRecords === 'function') {
                        publishedRecords = await calendarAdapter.getPublishedCalendarRecords(bucket.city || '');
                    }
                } catch (error) {
                    publishedRecords = null; // optional rule source only
                }
                const descriptors = [];
                for (const stamp of bucket.series.values()) {
                    const descriptor = this.resolveHygieneSeriesDescriptor(stamp, candidates, publishedRecords);
                    if (descriptor) descriptors.push(descriptor);
                }
                if (descriptors.length === 0) continue;
                findings.push(...this.findSupersededCalendarSingles(candidates, descriptors, calendarName));
            }
        } catch (error) {
            // Report-only: an empty (or partial) report, never a wrong one.
        }
        for (const finding of findings) {
            if (finding.kind === 'off-pattern') {
                console.log(`🧹 HYGIENE: "${finding.title}" (${finding.day}) in ${finding.calendarName} matches saved series "${finding.series.title}" but its date is OFF-PATTERN (${finding.series.rrule}${finding.reason ? `; ${finding.reason}` : ''}) — might be a genuinely special night; report-only, nothing is deleted`);
            } else {
                console.log(`🧹 HYGIENE: "${finding.title}" (${finding.day}) in ${finding.calendarName} looks superseded by saved series "${finding.series.title}" (${finding.series.rrule}; ${finding.series.instances} instance(s) in window) — report-only, nothing is deleted`);
            }
            if (finding.caution) {
                console.log(`🧹 HYGIENE: ⚠️ "${finding.title}" carries a manual bear verdict/review flag (${finding.cautionReason}) — extra caution before touching it`);
            }
        }
        if (findings.length > 0) {
            console.log(`🧹 HYGIENE: ${findings.length} calendar event(s) look superseded/off-pattern next to saved series — checklist only, deletion stays manual in the Calendar app`);
        }
        return findings;
    }

    // One confirmed-series stamp → an evaluable descriptor, or null when the
    // series cannot even be identity-matched (no record of it to compare
    // against). Rule sources, strongest first:
    //   1. The published calendar ICS VEVENT for the series UID (identifier
    //      suffix after the first colon IS the ICS UID): true DTSTART, RRULE
    //      and EXDATEs — COUNT/UNTIL are safe to honor from a true start.
    //   2. The wide-window instance's own notes `recurrence:` key (or an
    //      expanded record's `recurrence` field), anchored at the EARLIEST
    //      instance visible in the window — pattern-correct for the supported
    //      shapes, but not the true series start, so COUNT rules and
    //      pre-anchor dates are treated as unevaluable (fail closed).
    // A descriptor with no usable rule is still returned (rrule '') so its
    // identity keeps matching — the membership check then yields "uncertain"
    // and the matched singles are omitted rather than misjudged.
    resolveHygieneSeriesDescriptor(stamp, candidates, publishedRecords) {
        const identifier = String(stamp && stamp.identifier ? stamp.identifier : '').trim();
        ...
        for (const candidate of Array.isArray(candidates) ? candidates : []) {
            ...
            if (candidateIdentifier !== identifier) continue;
            if (candidate.startDate instanceof Date && !isNaN(candidate.startDate.getTime())) {
                instances.push(candidate);
            }
        }
        instances.sort((a, b) => a.startDate.getTime() - b.startDate.getTime());
        const earliestInstance = instances.length > 0 ? instances[0] : null;

        // Published-calendar series VEVENT for this UID (rule source 1).
        const uid = SharedCore.extractIcsUidFromIdentifier(identifier) || identifier;
        let publishedSeries = null;
        if (Array.isArray(publishedRecords)) {
            publishedSeries = publishedRecords.find(record => record && record.uid === uid &&
                record.rrule && !record.recurrenceId && record.start && record.start.date instanceof Date) || null;
        }

        // The record the identity gate compares singles against.
        const identityRecord = earliestInstance || (publishedSeries ? {
            identifier: publishedSeries.uid,
            title: publishedSeries.summary || '',
            startDate: publishedSeries.start.date,
            location: publishedSeries.location || '',
            notes: publishedSeries.description || ''
        } : null);
        if (!identityRecord) return null;

        const notesFields = this.parseNotesIntoFields(identityRecord.notes || '');
        const notesTimezone = typeof notesFields.timezone === 'string' ? notesFields.timezone.trim() : '';

        let rrule = '';
        let startRecord = null;
        let exdates = [];
        let anchorIsTrueStart = false;
        let ruleSource = '';
        let timezone = '';
        if (publishedSeries) {
            rrule = publishedSeries.rrule;
            startRecord = publishedSeries.start;
            exdates = Array.isArray(publishedSeries.exdates) ? publishedSeries.exdates : [];
            anchorIsTrueStart = true;
            ruleSource = 'published-calendar-ics';
            const tzid = publishedSeries.start.tzid && publishedSeries.start.tzid !== 'UTC'
                ? publishedSeries.start.tzid
                : '';
            timezone = tzid || notesTimezone;
        } else if (earliestInstance) {
            const statedRule = (typeof notesFields.recurrence === 'string' && notesFields.recurrence.trim())
                || (typeof earliestInstance.recurrence === 'string' && earliestInstance.recurrence.trim())
                || '';
            rrule = statedRule;
            anchorIsTrueStart = false;
            ruleSource = 'calendar-notes';
            timezone = notesTimezone || this.getCityTimezone(stamp.city) || '';
            // BYDAY names LOCAL weekdays: anchor the expansion at the
            // instance's wall clock IN THE SERIES TIMEZONE, not at its UTC
            // reading (a 9pm LA Thursday is a Friday in UTC). Unresolvable
            // timezone → null start record → membership stays uncertain and
            // the matched singles are omitted (fail closed).
            startRecord = SharedCore.instantToZonedWallClockRecord(earliestInstance.startDate, timezone);
        }
        if (!timezone) {
            timezone = this.getCityTimezone(stamp.city) || '';
        }

        return {
            identifier,
            title: identityRecord.title || stamp.title || '',
            instances: Math.max(instances.length, Number.isFinite(stamp.instances) ? stamp.instances : 0),
            identityRecord,
            rrule,
            startRecord,
            exdates,
    ...
    // A UTC instant → the parseIcsDateValue-shaped start record ({ date,
    // wall, tzid, isDateOnly }) of its wall clock IN an IANA timezone, so a
    // device-observed series instance can anchor an RRULE expansion whose
    // BYDAY names local weekdays. Null when the offset cannot be resolved
    // (callers fail closed). No timezone → read as UTC.
    static instantToZonedWallClockRecord(date, timezone) {
        if (!(date instanceof Date) || isNaN(date.getTime())) return null;
        const zone = typeof timezone === 'string' && timezone.trim() ? timezone.trim() : '';
        let offsetMinutes = 0;
        if (zone && !/^(UTC|GMT|Z)$/i.test(zone)) {
            offsetMinutes = SharedCore.getIcsTimezoneOffsetMinutes(date, zone);
            if (!Number.isFinite(offsetMinutes)) return null;
        }
    ...
    // Does the confirmed series' rule generate the single's date?
    //   'covered'       generated (and not EXDATE-excluded) — superseded.
    //   'excluded'      generated by the rule but EXDATE-excluded by the
    //                   saved series — that night is deliberately special.
    //   'not-generated' the rule does not produce this date.
    //   null            cannot be evaluated confidently — the caller OMITS.
    resolveHygieneDateMembership(descriptor, singleStart) {
        if (!descriptor || !(singleStart instanceof Date) || isNaN(singleStart.getTime())) return null;
        if (!descriptor.rrule || !descriptor.startRecord) return null;
        const timezone = descriptor.timezone || null;
        const singleDay = this.normalizeEventDateLocal(singleStart, timezone);
        if (!singleDay) return null;
        if (!descriptor.anchorIsTrueStart) {
            // Anchored at a mid-series instance, not the true DTSTART: COUNT
            // indexes from the real start (unknowable here), and dates before
            // the anchor cannot be generated by a forward expansion.
            const parts = SharedCore.parseRruleParts(descriptor.rrule);
            if (!parts || parts.COUNT !== undefined) return null;
            const anchorDate = descriptor.startRecord instanceof Date
                ? descriptor.startRecord
                : (descriptor.startRecord && descriptor.startRecord.date);
            if (!(anchorDate instanceof Date) || isNaN(anchorDate.getTime())) return null;
            const anchorDay = this.normalizeEventDateLocal(anchorDate, timezone);
            if (!anchorDay || singleDay < anchorDay) return null;
        }
        const DAY_MS = 24 * 60 * 60 * 1000;
        const windowStart = new Date(singleStart.getTime() - 2 * DAY_MS);
        const windowEnd = new Date(singleStart.getTime() + 2 * DAY_MS);
        const occurrences = SharedCore.expandRruleOccurrencesInWindow(
            descriptor.rrule, descriptor.startRecord, windowStart, windowEnd
        );
        if (occurrences === null) return null; // unsupported/unparseable rule
        const sameDayExdate = (Array.isArray(descriptor.exdates) ? descriptor.exdates : []).some(exdate =>
            exdate && exdate.date instanceof Date &&
            this.normalizeEventDateLocal(exdate.date, timezone) === singleDay);
        const generated = occurrences.some(occurrence =>
            this.normalizeEventDateLocal(occurrence, timezone) === singleDay);
        if (generated) return sameDayExdate ? 'excluded' : 'covered';
        // An EXDATE names an occurrence the series HAD on that day even when
        // the ±2d expansion window missed it (timezone skew) — still special.
        if (sameDayExdate) return 'excluded';
        return 'not-generated';
    }

    // Pure scan: wide-window candidates + confirmed-series descriptors →
    // hygiene findings for one calendar. Exclusion ladder (each rung fails
    // closed toward "not listed"):
    //   1. no/invalid start date
    //   2. single-occurrence OVERRIDES (notes/props overrideUid or
    //      overrideRecurrenceId — the slot-host mechanism; deliberate)
    //   3. series records themselves (notes `recurrence:` key or a
    //      `recurrence` field)
    //   4. series INSTANCES (≥2 wide-window events share the identifier — the
    //      same evidence resolveSeriesProbeDecision counts)
    //   5. any candidate sharing a confirmed series' identifier (its anchor)
    //   6. identity matches nothing confirmed → not our business
    //   7. ANY matched series evaluates uncertain → omitted entirely
    findSupersededCalendarSingles(candidates, descriptors, calendarName) {
        const findings = [];
        if (!Array.isArray(candidates) || !Array.isArray(descriptors) || descriptors.length === 0) {
            return findings;
        }
        const seriesIdentifiers = new Set(descriptors.map(descriptor => descriptor.identifier));
        for (const candidate of candidates) {
            if (!candidate) continue;
            if (!(candidate.startDate instanceof Date) || isNaN(candidate.startDate.getTime())) continue;
            const fields = this.parseNotesIntoFields(candidate.notes || '');
            if (fields.overrideUid || fields.overrideRecurrenceId ||
                candidate.overrideUid || candidate.overrideRecurrenceId) {
                continue; // deliberate single-occurrence override — never listed
            }
            if ((typeof fields.recurrence === 'string' && fields.recurrence.trim()) ||
                (typeof candidate.recurrence === 'string' && candidate.recurrence.trim())) {
                continue; // a series record, not a single
            }
            const identifier = candidate.identifier === null || candidate.identifier === undefined
                ? ''
                : String(candidate.identifier).trim();
            if (identifier && seriesIdentifiers.has(identifier)) continue; // the series' own record
            const probe = SharedCore.resolveSeriesProbeDecision(candidates, identifier);
            if (probe.instanceCount >= 2) continue; // an instance of a device series
            const matched = descriptors.filter(descriptor =>
                this.matchesSavedSeriesIdentity(candidate, descriptor.identityRecord));
            if (matched.length === 0) continue;
            let uncertain = false;
            let coveredBy = null;
            const offPattern = [];
            for (const descriptor of matched) {
                const membership = this.resolveHygieneDateMembership(descriptor, candidate.startDate);
                if (membership === null) { uncertain = true; break; }
                if (membership === 'covered') { coveredBy = coveredBy || descriptor; }
                else { offPattern.push({ descriptor, membership }); }
            }
            if (uncertain) continue; // fail closed: no claim beats a wrong claim
            const bearSource = typeof fields.bearSource === 'string' ? fields.bearSource.trim() : '';
            const cautionReasons = [];
            if (this.isManualBearSource(bearSource)) cautionReasons.push(`bearSource: ${bearSource}`);
            if (typeof fields.bearReview === 'string' && fields.bearReview.trim()) {
                cautionReasons.push(`bearReview: ${fields.bearReview.trim()}`);
            }
            if (fields.manuallyMarkedBear) cautionReasons.push('manuallyMarkedBear');
            const timezoneForDay = (coveredBy || offPattern[0].descriptor).timezone || null;
            const base = {
                title: candidate.title || 'Untitled event',
                startDate: candidate.startDate,
                day: this.normalizeEventDateLocal(candidate.startDate, timezoneForDay),
                calendarName: String(calendarName || ''),
                identifier,
                caution: cautionReasons.length > 0,
            ...
            if (coveredBy) {
                findings.push({
                    ...
                    kind: 'superseded',
                    series: {
                        identifier: coveredBy.identifier,
                        title: coveredBy.title,
                        rrule: coveredBy.rrule,
                        instances: coveredBy.instances,
                        ruleSource: coveredBy.ruleSource
                    },
                    reason: `its date ${base.day} is generated by ${coveredBy.rrule}`
                });
            } else {
                const first = offPattern[0];
                findings.push({
                    ...base,
                    kind: 'off-pattern',
                    series: {
                        identifier: first.descriptor.identifier,
                        title: first.descriptor.title,
                        rrule: first.descriptor.rrule,
                        instances: first.descriptor.instances,
                        ruleSource: first.descriptor.ruleSource
                    },
                    reason: first.membership === 'excluded'
                        ? `the saved series explicitly excludes ${base.day} (EXDATE)`
                        : `its date ${base.day} is not generated by ${first.descriptor.rrule}`
                });
            }
        }
        return findings;
