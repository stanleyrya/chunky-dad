...
class ScriptableAdapter {
    ...
    async writeCachedOcrSummary(htmlUrl, ocrConfig, ocrResults, cacheHelpers) {
        ...
        const documentsDir = fm.documentsDirectory();
        const baseDir = fm.joinPath(fm.joinPath(fm.joinPath(documentsDir, 'chunky-dad-scraper'), 'storage'), 'ocr');
        const cacheDir = fm.joinPath(baseDir, hostDir);
        const cachePath = fm.joinPath(cacheDir, `ocr-summary--${fileName}`);
        ...
        try {
            ...
            fm.writeString(cachePath, JSON.stringify(summary, null, 2));
    ...
    async readCachedOcrSummary(htmlUrl, ocrConfig, cacheHelpers) {
        ...
        const documentsDir = fm.documentsDirectory();
        const baseDir = fm.joinPath(fm.joinPath(fm.joinPath(documentsDir, 'chunky-dad-scraper'), 'storage'), 'ocr');
        const cachePath = fm.joinPath(fm.joinPath(baseDir, hostDir), `ocr-summary--${fileName}`);
        ...
        try {
            ...
            const fileAttributes = fm.fileAttributes(cachePath);
            const modificationDate = fileAttributes?.NSFileModificationDate || new Date();
            const now = new Date();
            const ageMs = now - modificationDate;
    ...
    // Parse URL components (works in Scriptable with URL class)
