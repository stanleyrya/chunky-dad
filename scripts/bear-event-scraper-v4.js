// Bear Event Scraper v4 for Scriptable (Latest Version - Precise Parsing + Preview)
// Most accurate parsing based on actual website structure analysis
// Includes preview mode and full safety controls (DRY_RUN, PREVIEW_MODE, CALENDAR_SYNC_ENABLED)
...
class BearEventScraperV3 {
    constructor() {
        ...
        // Configuration files
            this.CONFIG_FILE = "bear-event-scraper-v4-config.json";
    this.INPUT_FILE = "bear-event-parser-input.json";
    this.LOG_FILE = "bear-event-scraper-v4-logs.txt";
    this.PERFORMANCE_FILE = "bear-event-scraper-v4-performance.csv";
    ...
    async parseAllSources() {
        ...
        if (this.PREVIEW_MODE) {
            ...
            const previewPath = fm.documentsDirectory() + `/bear-event-scraper-v4-preview-${timestamp}.txt`;
    ...
    generateReport(results) {
        ...
        const timestamp = new Date().toISOString().split('T')[0];
        const fm = FileManager.local();
        const reportPath = fm.documentsDirectory() + `/bear-event-scraper-v4-report-${timestamp}.txt`;
