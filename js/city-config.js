...
// Check if city has bars enabled
function hasCityBars(cityKey) {
    const config = getCityConfig(cityKey);
    return config && config.barsEnabled;
}

// Get cities with bars enabled
function getCitiesWithBars() {
    return Object.keys(CITY_CONFIG)
        .map(key => ({
            key,
            ...CITY_CONFIG[key]
        }))
        .filter(city => city.visible !== false && city.barsEnabled !== false);
}
...
if (typeof window !== 'undefined') {
  ...
  window.hasCityBars = hasCityBars;
  window.getCitiesWithBars = getCitiesWithBars;
}

// Export for use in other modules
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { CITY_CONFIG, getCityConfig, getAvailableCities, hasCityCalendar, hasCityBars, getCitiesWithBars };
}